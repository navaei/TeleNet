...
namespace TeleNet.Models
{
    ...
    {
        private const string defaultConnectionAddress = "149.154.167.50";//"149.154.175.100";
        ...
        public TLAbsUser TLUser { get; set; }
        ...
        public static Session FromBytes(byte[] buffer, ISessionStore store, string sessionUserId)
        {
            ...
            using (var reader = new BinaryReader(stream))
            {
                ...
                TeleNet.Models.TL.TLAbsUser TLUser = null;
                if (isAuthExsist)
                {
                    sessionExpires = reader.ReadInt32();
                    TLUser = (TeleNet.Models.TL.TLAbsUser)ObjectUtils.DeserializeObject(reader);
                }
        ...
        public static Session TryLoadOrCreateNew(ISessionStore store, string sessionUserId, string connectionAddress = "", int port = defaultConnectionPort)
        {
            return store.Load(sessionUserId) ?? new Session(store)
            {
                Id = GenerateRandomUlong(),
                SessionUserId = sessionUserId,
                ServerAddress = string.IsNullOrEmpty(connectionAddress) ? defaultConnectionAddress : connectionAddress,
                Port = port
            };
        }
